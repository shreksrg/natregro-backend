	...
	public function post() {
		...
		if (strpos($url,APP_PATH) === 0) {
			...
			$domain = $urls['scheme'].'://'.$urls['host'].(isset($urls['port']) && !empty($urls['port']) ? ":".$urls['port'] : '').'/';
                ...
                $contact_info = isset($_POST['contact_info']) && trim($_POST['contact_info']) ? trim($_POST['contact_info']) : $this->_show_msg(L('请输入联系方式'), HTTP_REFERER);
                $username = isset($_POST['username']) && trim($_POST['username']) ? trim($_POST['username']) : $this->_show_msg(L('请输入姓名'), HTTP_REFERER);
		...
		$direction = isset($_POST['direction']) && intval($_POST['direction']) ? intval($_POST['direction']) : '';
                ...
                $data = array('userid'=>$userid, 'username'=>$username, 'content'=>$content, 'direction'=>$direction, 'contact_info' => $contact_info);
